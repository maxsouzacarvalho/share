#!/bin/bash

PEM="-----BEGIN CERTIFICATE-----
\n
-----END CERTIFICATE-----"

# Função para instalar certificado
instalar_certificado() {
    local cert_path="$1"

    if [ ! -f "$cert_path/Appliance-Certificate.pem" ]; then
        echo "[INFO] Instalando certificado em $cert_path/Appliance-Certificate.pem"

        # Salvar PEM preservando quebras de linha
        echo "$PEM" > "$cert_path/Appliance-Certificate.pem"

        # Atualizar certificados
        if command -v update-ca-certificates &>/dev/null; then
            update-ca-certificates
        elif command -v update-ca-trust &>/dev/null; then
            update-ca-trust extract
        fi
    else
        echo "[INFO] Certificado já existe em $cert_path/Appliance-Certificate.pem"
    fi
}

# Debian/Ubuntu
if [ -d /usr/local/share/ca-certificates/ ]; then
    instalar_certificado "/usr/local/share/ca-certificates"
fi

# RHEL/CentOS/Rocky
if [ -d /etc/pki/ca-trust/source/anchors/ ]; then
    instalar_certificado "/etc/pki/ca-trust/source/anchors"
fi
