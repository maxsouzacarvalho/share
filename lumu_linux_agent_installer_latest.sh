...
INSTALLATION_SCRIPT_VERSION="v2.1.0"
...
print_help_message() {
    ...
    echo "                           using the LICENSE as activation code."
    echo "  -r, --repo               Only installs the Lumu Linux repository."
...
rpm_based_install_repository() {
    RPM_REPO_FILE="/etc/yum.repos.d/lumu.repo"
    RPM_BASED_NAME=$1

    if [ -f "$RPM_REPO_FILE" ]; then
        warning "The Lumu Linux repository is already installed. Checking for modifications."
        if [ "$RPM_BASED_NAME" = "CentOS Stream" ] || [ "$RPM_BASED_NAME" = "Oracle Linux Server" ]; then
            info "Modifying existing repository configuration for $RPM_BASED_NAME."
            sed -i 's/\$releasever/8/g' "$RPM_REPO_FILE"
        fi
        return 0
    fi
    ...
    if [ "$OS" = "CentOS Stream" ] || [ "$OS" = "Oracle Linux Server" ]; then
        info "Modifying repository configuration for $OS."
        sed -i 's/\$releasever/8/g' "$RPM_REPO_FILE"
    fi
}
...
rpm_based_install() {
    ...
    rpm_based_install_repository "$RPM_BASED_NAME"

    info "Installing the Lumu Linux Agent package."
    LANG="C.UTF-8" yum install -q -y "$LUMU_PACKAGE_NAME" >/dev/null 2>&1
}

suse_based_install_repository() {
    info "Installing Lumu repository."
    zypper --gpg-auto-import-keys -q addrepo "$BASE_REPOSITORY_URL/sles/lumu.repo"
}

suse_based_install_repository() {
    local repo_name="lumu"
    
    if zypper repos | grep -q "$repo_name"; then
        warning "Repository '$repo_name' already exists. Skipping repository installation."
        return 0
    fi
    
    info "Installing Lumu repository."
    zypper --gpg-auto-import-keys -q addrepo "$BASE_REPOSITORY_URL/sles/$repo_name.repo"
}
...
apt_based_install_repository() {
    APT_BASED_NAME=$1

    if [ -d "/etc/apt/sources.list.d/lumu.list" ]; then
        warning "The Lumu Linux repository is already installed. Omitting this step."
    fi
    ...
    COMPONENT="main"
    echo "deb [arch=$ARCH signed-by=/etc/apt/keyrings/lumu.gpg] $(. /etc/os-release && echo "$BASE_REPOSITORY_URL/$APT_BASED_NAME") $CODENAME $COMPONENT" | tee /etc/apt/sources.list.d/lumu.list >/dev/null
...
apt_based_install() {
    ...
    apt_based_install_repository "$APT_BASED_NAME"
...
if [ "$OPERATION" = "uninstall" ]; then
    ...
    "Ubuntu" | "Debian GNU/Linux" | "KDE neon")
        apt_based_uninstall
        ;;
    "Fedora Linux" | "Red Hat Enterprise Linux" | "Oracle Linux Server" | "CentOS Stream")
        rpm_based_uninstall
        ;;
    "SLES" | "openSUSE Leap")
        suse_based_uninstall
    ...
    *)
    ...
    "SLES" | "openSUSE Leap")
        ...
        suse_based_install
        ;;
    "Red Hat Enterprise Linux" | "Oracle Linux Server" | "CentOS Stream")
        rpm_based_install_dependencies
    ...
    *)
    ...
    info "Done!"
elif [ "$OPERATION" = "install-repository" ]; then
    info "Installing the Lumu Linux repository for $OS $VER..."

    case $OS in
    "Ubuntu" | "KDE neon")
        apt_based_install_repository "ubuntu"
        ;;
    "Debian GNU/Linux")
        apt_based_install_repository "debian"
        ;;
    "Fedora Linux")
        rpm_based_install_repository "fedora"
        ;;
    "SLES" | "openSUSE Leap")
        suse_based_install_repository
        ;;
    "Red Hat Enterprise Linux" | "Oracle Linux Server" | "CentOS Stream")
        rpm_based_install_repository "rhel"
        ;;
    *)
        error "Platform $OS $VER is not supported by the Lumu Linux Agent installer."
        error "Please, fallback to manual installation process at <https://docs.lumu.io/portal/en/kb/articles/lumu-agent-for-linux>"
